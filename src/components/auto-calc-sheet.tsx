...
import { BATTERY_TYPES, PANEL_TYPES } from "@/data/estimate";
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="shrink-0 border-b border-border bg-background pb-4">
        ...
        <p className="mt-1 text-sm font-semibold uppercase tracking-wide text-brand">Bảng tính AUTO</p>
        <div className="mt-3 flex flex-wrap items-end justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            Bảng tính AUTO — nhập ô xanh, số đỏ là kết quả. Giá điện {money(form.tariff)} đ/kWh (chưa
            thuế).
          </p>
          <Button
            type="button"
      ...
      <Sheet title="1. Thông tin tiền điện">
        <table className="w-full table-fixed border-collapse text-xs">
          ...
          <thead>
            <YellowHead
              cols={["Thông tin", "Số tiền điện (chưa thuế)", "Số kW phải mua"]}
            />
          </thead>
          <tbody>
            ...
            <tr className="border-t border-border">
              ...
              <td className="px-3 py-1.5 text-right text-muted-foreground">kWh = tiền / đơn giá</td>
            </tr>
          </tbody>
        </table>
      </Sheet>
              ...
              <td colSpan={2} className="px-2 py-1">
                <div className="flex items-center gap-2">
                  <NumInput
                    ...
                    className="max-w-[8rem]"
                  />
                  <span className="text-[11px] text-muted-foreground">Đề xuất {r.suggestedQty}</span>
                </div>
              </td>
            </tr>
            <tr className="border-t border-border">
